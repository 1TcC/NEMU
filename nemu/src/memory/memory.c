...
static CacheLine *cache_fetch(hwaddr_t addr) {
	...
	for(i = 0; i < CACHE_BLOCK_SIZE; i += 4) {
		uint32_t data = dram_read(block_addr + i, 4);
...
static uint32_t cache_read(hwaddr_t addr, size_t len) {
	...
	while(done < len) {
		...
		uint32_t offset = cur_addr & (CACHE_BLOCK_SIZE - 1);

		size_t part_len = CACHE_BLOCK_SIZE - offset;
		if(part_len > len - done) {
			part_len = len - done;
		}

		CacheLine *line = cache_fetch(cur_addr);
...
static void cache_write(hwaddr_t addr, size_t len, uint32_t data) {
	...
	while(done < len) {
		...
		uint32_t set = (cur_addr >> 6) & 0x7f;
		uint32_t tag = cur_addr >> 13;
		uint32_t offset = cur_addr & (CACHE_BLOCK_SIZE - 1);
